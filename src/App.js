...
class App extends Component {
  constructor() {
    ...
    this.state = {
      ...
      share: {
        title: "Sham Tartan",
        description:
          "A Sham (pretend, unofficial) Tartan from http://shartan.houseofmoran.com/",
  ...
  handleShartanClick() {
    ...
    targetDate.setTime(startDate.getTime() + transitionDuration);
    const target = this.settGen.settFrom(`${targetDate.toISOString()}`);

    const transitionFn = this.settInterp.asFunction(
      Sett.parse(this.state.sett),
      target
    );
    ...
    const updateStateFn = () => {
      ...
      if (currentDate < targetDate) {
        const t =
          (currentDate.getTime() - startDate.getTime()) / transitionDuration;
        this.setState({
    ...
    return (
      <div className="App">
        <Shartan
          ...
          width={400}
          onClick={this.handleShartanClick}
        />
        <input
